...
export default function ResultsDisplay() {
  ...
  const customerApproval = useSelector(
    (state) => state.settings.customerApproval
  );
  ...
  if (!results || !results.objects || results.objects.length === 0) {
    return (
      <div className="card">
        <p>
          Keine Berechnungsergebnisse verfügbar. Bitte fügen Sie Objekte und
          Leistungen hinzu.
        </p>
      </div>
    );
      ...
      <div className="card">
          ...
          return (
            ...
            >
              ...
              <div style={{ marginBottom: "15px" }}>
                ...
                <ul>
                  <li>
                    Deckenfläche: {obj.quantities?.ceilingArea?.toFixed(2) || 0}{" "}
                    m²
                  </li>
                  <li>
                    Umfang: {obj.quantities?.perimeter?.toFixed(2) || 0} m
                  </li>
                  <li>
                    Wandfläche: {obj.quantities?.wallArea?.toFixed(2) || 0} m²
                  </li>
                  ...
                  >
                    {obj.specialNotes.map((note) => {
                      const requiredServiceTitle = note.requiredService
                        ? services.find((s) => s.id === note.requiredService)
                            ?.title
                        : null;
        ...
        >
          ...
          >
            ...
            <div>
              <strong>
                Stundenlohn: {formatCurrency(companySettings?.laborRate || 65)}
                /h
              </strong>
                  ...
                  >
                    ...
                    >
                      {obj.specialNotes.map((note, noteIdx) => {
                        ...
                        const isSeparatelyCharged =
                          !hasFactor && !hasRequiredService;
                  ...
                  obj.services.map((svc, idx) => (
                    <div
                      ...
                      style={{
                        ...
                        paddingBottom: "12px",
                        borderBottom:
                          idx < obj.services.length - 1
                            ? "1px dotted #e0e0e0"
                            : "none",
                        marginLeft: svc.isSubService ? "20px" : "0",
                        borderLeft: svc.isSubService
                          ? "3px solid #1976d2"
                          : "none",
                        paddingLeft: svc.isSubService ? "12px" : "0",
                        background: svc.isSubService
                          ? "#f8fbff"
                          : "transparent",
                      }}
                    >
                      ...
                      >
                        ...
                        >
                          ...
                          {svc.standardValuePerUnit ? (
                            <div style={{ color: "#555" }}>
                              {svc.standardValuePerUnit} min/{svc.unit} ×{" "}
                              {svc.quantity?.toFixed(2)} {svc.unit} ={" "}
                              {svc.calculatedBaseTime?.toFixed(2) ||
                                (
                                  svc.standardValuePerUnit * svc.quantity
                                )?.toFixed(2)}{" "}
                              min
                            </div>
                          ) : (
                          ...
                          {svc.hasBundledServices && svc.bundledServices && (
                            <div
                              style={{
                                color: "#7b1fa2",
                                marginTop: "6px",
                            ...
                            >
                              ...
                              >
                                📦 Gebündelte Unterleistungen (Zeit kombiniert):
                              </div>
                              {svc.bundledServices.map((bundled, bIdx) => (
                                <div
                                  key={bIdx}
                                  style={{
                                    marginLeft: "8px",
                                ...
                                >
                                  + {bundled.title}:{" "}
                                  {bundled.standardValuePerUnit} min/{svc.unit}{" "}
                                  × {bundled.quantity?.toFixed(2)} {svc.unit} ={" "}
                                  {bundled.time?.toFixed(2)} min
                                </div>
                              ))}
                              <div
                                style={{
                              ...
                              >
                                = Kombinierte Basiszeit:{" "}
                                {svc.baseTime?.toFixed(2)} min
                              </div>
                            </div>
                          )}
                          ...
                          {svc.minTimeApplied && (
                            ...
                            >
                              ⚠️ Mindestzeit angewendet: {svc.minTime} min
                              (statt {svc.calculatedBaseTime?.toFixed(2)} min)
                            </div>
                          )}

                          {/* Sonderfaktor */}
                          {svc.specialNoteFactor &&
                            svc.specialNoteFactor !== 1 && (
                              <div
                                style={{
                                  color: "#e65100",
                                  marginTop: "4px",
                              ...
                              >
                                🔧 Sonderangaben-Faktor: ×
                                {svc.specialNoteFactor} (+
                                {((svc.specialNoteFactor - 1) * 100).toFixed(0)}
                                %)
                              </div>
                            )}
                          ...
                          {svc.efficiency && svc.efficiency !== 1 && (
                            ...
                            >
                              ...
                              {svc.efficiencyStart &&
                                ` (ab ${svc.efficiencyStart} ${svc.unit})`}
                            </div>
                          )}
                        ...
                        >
                          <span style={{ color: "#555" }}>
                            💼 Lohnkosten: {formatTime(svc.finalTime)} ×{" "}
                            {formatCurrency(companySettings?.laborRate || 65)}/h
                          </span>
                        ...
                        {svc.materialType && svc.materialType !== "none" && (
                          ...
                          >
                            <span style={{ color: "#555" }}>
                              📦 Material:{" "}
                              {svc.materialType === "percent"
                                ? `${svc.materialValue}% von ${formatCurrency(
                                    svc.laborCost
                                  )}`
                                : `${formatCurrency(svc.materialValue)}/${
                                    svc.unit
                                  } × ${svc.quantity?.toFixed(2)}`}
                            </span>
                            <span style={{ fontWeight: "500" }}>
                              = {formatCurrency(svc.materialCost)}
                            </span>
              ...
              >
                {results.siteSetupCost > 0 && (
                  ...
                  >
                    <span>
                      🚚 Einrichtung:{" "}
                      {formatTime(companySettings?.siteSetup || 60)} ×{" "}
                      {formatCurrency(companySettings?.laborRate || 65)}/h
                    </span>
                    <span style={{ fontWeight: "bold" }}>
                ...
                {results.siteClearanceCost > 0 && (
                  ...
                  >
                    <span>
                      🧹 Räumung:{" "}
                      {formatTime(companySettings?.siteClearance || 60)} ×{" "}
                      {formatCurrency(companySettings?.laborRate || 65)}/h
                    </span>
                    <span style={{ fontWeight: "bold" }}>
        ...
        <div className="card">
          ...
          >
            ...
            >
              ...
              >
                Für Leistungen mit Trocknungszeiten wurden Wartezeiten
                berücksichtigt. Inkompatible Arbeiten wurden auf verschiedene
                Tage verteilt.
              </p>
            </div>
            ...
            >
              ...
              >
                Die Leistungen wurden nach der definierten Arbeitsreihenfolge
                sortiert, um einen logischen Arbeitsablauf zu gewährleisten.
              </p>
            </div>
            ...
            >
              ...
              >
                Die optimale Mitarbeiteranzahl wurde berechnet, um
                Effizienzverluste zu vermeiden und die Gesamtarbeitszeit zu
                optimieren.
              </p>
              <div
                style={{
              ...
              >
                <strong>Ergebnis:</strong> {results.optimalEmployees || 1}{" "}
                Mitarbeiter für {results.totalDays || 0} Arbeitstage
              </div>
            </div>
          </div>
